<header>
    <div class="container relative h-[66px] flex items-center">
        <a href="{{ route('home') }}" class="block absolute top-0 left-0 px-4 sm:px-8 xl:px-4 2xl:px-8 min-h-[72px]" title="{{ __('topbar.logo') }}">
        ...
        <ul class="h-[66px] ms-auto flex items-center [&>li]:ms-6">
            <li>
                <a href="{{ route('search') }}" class="flex items-center justify-center w-9 h-9 rounded-full ring-2 ring-slate-300 hover:ring-slate-400 bg-slate-50" title="{{ __('topbar.search') }}">
            ...
            <li>
                <label for="language" class="block cursor-pointer" title="{{ __('topbar.language') }}">
