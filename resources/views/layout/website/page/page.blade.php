...
@section('content')
    ...
    <article data-id="{{ $page->id }}">
            ...
            <div class="grid justify-center w-full max-w-[800px] mx-auto">
                <div class="grid gap-2 md:gap-3 justify-center px-3">
                    ...
                    <ul class="mx-auto sm:marker:text-slate-400 sm:list-disc sm:space-x-5 items-center flex flex-col sm:flex-row text-slate-600">
                        <li class="list-none sm:pe-3">
                            {{ __('website.page.author') }} {{ $page->author()->name }}
                        </li>
                        <li>
                ...
                @if($page->image)
                    <picture>
                        ...
                        <img fetchpriority="high" class="mt-3 md:mt-5 lg:h-[382px] block lg:rounded-md object-cover" width="800" height="382" src="{{ asset('storage/images/hero/'.$page->slug.'.jpg') }}" alt="{{ __('website.image.representing', ['image' => strtolower($page->title)]) }}">
                    </picture>
                @else
                ...
                <div class="flex items-center justify-between p-3">
                    <span>
                        {{ __('website.page.read', ['minute' => $page->reading_time]) }}
                    </span>
                    ...
                    <section id="{{$type->type}}" class="grid gap-3 xl:gap-6 mt-5 xl:mt-12">
                        ...
                        <ol role="list" class="grid gap-4 {{ $things->where('type_id', $type->id)->count() === 1 ? '' : 'md:grid-cols-2' }} xl:gap-6">
                            @foreach($things->where('type_id', $type->id) as $thing)
                                @include('layout.website.include.thing')
                            @endforeach
                        </ol>
                        @if($type->id === 1)
                            <p class="text-center text-slate-500">
                                {{ __('website.page.legal') }}
                            </p>
                        @endif
                    </section>
                @endif
        ...
        @if(!$tips->isEmpty())
            <section id="{{ __('website.page.tips.id') }}" class="bg-amber-50 py-10 mt-10 xl:mt-12">
                <div class="container">
                    @php
                        $cols = '';
        ...
        @endif
        @if(!$sections->isEmpty())
            <section id="{{ __('website.page.steps.id') }}" class="bg-teal-100 py-10 {{ $tips->isEmpty() ? 'mt-10 xl:mt-14' : 'mt-8 xl:mt-10' }} relative overflow-x-hidden">
        ...
        <div class="container mt-8 xl:mt-10">
            ...
            <footer class="grid gap-3 mt-8 md:mt-10">
                ...
                <p class="mt-6 md:mt-8 text-slate-500">
                    {{ __('website.page.disclaimer') }}
                </p>
                <p class="text-slate-500">
                    {{ __('website.page.amazon.disclaimer') }}
                </p>
            </footer>
        </div>
    </article>
    @if(!$related->isEmpty())
        <aside id="{{ __('website.page.related.id') }}" class="bg-slate-100 py-10 mt-8 xl:mt-10">
